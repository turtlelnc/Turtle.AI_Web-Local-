...
describe('TeamAction', () => {
  ...
  it('loads roster/task diagnostics on open and navigates a healthy teammate', async () => {
    ...
    expect(screen.getByText('进展正常')).toBeTruthy()
    expect(screen.getByText('目前不需要你处理')).toBeTruthy()
    expect(screen.getByText('任务进行中: Implement runtime')).toBeTruthy()
    expect(screen.getByText('下一项')).toBeTruthy()
    expect(screen.getByText('暂无排队任务')).toBeTruthy()
