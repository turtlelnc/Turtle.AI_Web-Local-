...
| `@deepseek-ai/dsh-experimental-tool-agent-team` | `interrupt_agent`、`list_agents`、`send_message`、`spawn_teammate`、`spawn_user_tester`、`team_task_create`、`team_task_get`、`team_task_list`、`team_task_update`、`wait_agent` | `ctx.tools`、`ctx.systemPrompt`、`ctx.agentTeams`、`an exact live Team member Agent` | `tool/call`、`team/member`、`team/message/queued`、`team/message/delivered`、`team/task`、`tool/result` | - | 这 10 个工具限定于隐式 Team Lead 与持久 teammate 作用域。随产品发布的 dsh-base bundle 默认禁用该包；文档中的 Agent Teams profile patch 会启用它，并禁用旧 continuable child 的同名控制工具。 |
...
创建一个使用 fresh context、执行固定中性任务的持久首次用户评估者。只有 Team Lead 可以调用此工具。评估者只获得应用入口，不会获得项目历史、产品意图、源码说明、规定工作流或偏好结论。
...
```json
{
  ...
  "properties": {
    "name": {
      "type": "string",
      "description": "Unique lower-kebab-case tester name."
    },
    "target": {
      "type": "string",
      "description": "Application entry visible to an ordinary user, such as a URL or executable path. Do not include product goals or usage instructions."
    }
  },
  "required": [
    "name",
